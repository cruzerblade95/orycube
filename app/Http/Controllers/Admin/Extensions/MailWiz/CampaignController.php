    ...
    public function index()
{
    ...
    $totalEmailsSent = $this->totalEmailsSent();
    $activeCampaignsCount = $this->activeCampaignsCount();
    $completedCampaignsCount = $this->completedCampaignsCount();

    return view('extensions.admin.mailwiz.campaigns.index', compact('campaigns', 'users', 'page_title', 'totalEmailsSent', 'activeCampaignsCount','completedCampaignsCount'));}
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'speed.required' => 'Please enter a speed.',
        ]);
    ...
    public function totalEmailsSent($returnAsView = false)
{
    $campaigns = Campaign::all();
    $totalEmailsSent = 0;
    ...
    foreach ($campaigns as $campaign) {
        ...
        if (is_array($done_targets)) {
            $totalEmailsSent += count($done_targets);
        }
    }

    if ($returnAsView) {
        return view('total-emails', ['totalEmailsSent' => $totalEmailsSent]);
    } else {
...
{
    $activeCampaignsCount = Campaign::where('status', Campaign::STATUS_ACTIVE)->count();
    return $activeCampaignsCount;
}

public function completedCampaignsCount()
{
    $completedCampaignsCount = Campaign::where('status', Campaign::STATUS_COMPLETED)->count();
